...
{
    public class UnidadGrupoDaoImplement : IUnidadGrupoDAO
    {
        public (List<DUnidadGrupo> lista, string error) ListaTodos()
        {
            List<DUnidadGrupo> lista = new List<DUnidadGrupo>();
            ...
            try
            {
                using (var db = new MiDbContext())
                {
                    ...
                    lista = db.UnidadesGrupo
                     .OrderBy(m => m.orden)   // Orden ascendente
                     .ToList();
